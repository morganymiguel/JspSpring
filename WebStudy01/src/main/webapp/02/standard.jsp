...
<pre>
	...
	현재 사용하고있는 소스정보: 
		...
		1. 정적 텍스트
			THML, CSS, JS, text
		2. 스크립틀릿: 서버상에서 실행된다.
			1) directive(지시자) : JSP 페이지에 대한 기본 설정 정보(page - required(필수)). &lt;%@ %&gt; &lt;@ 속성 %&gt;현재 페이지에 대한 설정을 작성하는 코드, 
				include: 정적 내포.
				taglib: 커스텀 태그 로딩.  			
			2) scriptlet : &lt;% %&gt;
				<% 
			...
			4) declaration(선언부) : &lt;%! %&gt;
			<%!private void test(){} private String txt; %>
			5) comment(주석) : <!--  --> <%--이거는 HTML --%>
...
/*				body{}  */
			...
			5가지의 토큰을 구분하는 방식이 다르다.
		3: 기본 객체
		4. action tag
		5. EL(Expression Language, 표현 연어)
